...
value <- c()
for(i in 1:100){
  value <- append(value,cos(i*(2*pi/24)))
}
print("Hello")
print(value)
...
print(x)  
  ...
  # Frequrency having amplitude peak 
  ...
  #if(plot.mode)lines(ti,start[1] * cos(2 * pi * ti/start[3] + start[2]* pi) + cons,col=4,lty="dashed")
...
CA.peaks <- function (x, span = 3, strict = TRUE) 
{
  ...
  z <- .Fortran("splus2rpeaks", as.double(x), as.integer(halfspan), 
                as.logical(strict), as.integer(dx[1]), as.integer(dx[2]), 
                double(length(x)))[[6]]
  mode(z) <- "logical"
  dim(z) <- dx
  if (dfp) {
...
CA.fftNllsVec(value,1.0,conf=0.05,maxiter=50,main=c(),plot.mode=T,lim=25,range=c(15,35),lwd=1,ylim=c(-1,1))
